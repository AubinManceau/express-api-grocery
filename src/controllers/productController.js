...
const getProducts = async (req, res) => {
    try{
        ...
        const detailledProduct = await Promise.all(products.map(async product => {
            const toSale = await ToSales.findByPk(product.toSale_id, {where: { deletedAt: null }});
...
const createProduct = async (req, res) => {
    try{
        const { name, description, image, HT_price, TTC_price, margin, selling_price, category, brand, weight, nb_soldBy, nb_stock, user_id_supplier } = req.body;
        const toSale = await ToSales.create({ name, description, image, HT_price, TTC_price, margin, selling_price });
        const product = await Products.create({ category, brand, weight, nb_soldBy, nb_stock, user_id_supplier, toSale_id: toSale.toSale_id });
...
const updateProduct = async (req, res) => {
    ...
    try{
        ...
        if(!product){
            ...
            const { name, description, image, HT_price, TTC_price, margin, selling_price, category, brand, weight, nb_soldBy, nb_stock, user_id_supplier } = req.body;
            const toSale = await ToSales.findByPk(product.toSale_id);
            await toSale.update({ name, description, image, HT_price, TTC_price, margin, selling_price });
            await product.update({ category, brand, weight, nb_soldBy, nb_stock, user_id_supplier });
            res.status(200).json({ product, toSale });
        }
    }catch(error){
